...
parser.add_argument('--iou-threshold', type=int, default=0.6)
...
for _ in range(args.nruns):
    ...
    train = 'CUDA_VISIBLE_DEVICES='  + args.gpu + ' python train.py --cuda --debug --save ' + model_name + ' --labels ' + args.labels + ' --data ' + args.data + ' --features ' + args.features + ' --vid-ids ' + str(args.vid_ids) + ' --epochs ' + str(args.epochs) + ' --batch-size 128 --max-W 1 --nthreads 1 --K 128 --W 256 --num-proposals 100 --iou-threshold ' + str(args.iou_threshold) + ' --log-interval 50 --dropout ' + params['dropout'] + ' --lr ' + params['lr'] + ' --weight-decay ' + params['weight-decay']
    print '^'*89
    print train
